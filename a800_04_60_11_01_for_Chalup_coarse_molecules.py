...
from Dmax_computation16 import *
...
if What_is_limiting==0:
    ...
    #for N limiting data
    from Chalup90_data_00 import chalup90

DPI = 600
#AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA
#Function beging here
#AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA
def kkI(data):   #this t00function calculate for the same irradiance
    print(data.i)
    I=data.Lightintensity
    if data.i ==0:
        plotcolor='#3953A9'
    elif data.i ==1:
        plotcolor='#EE2C3B'
    ...
    #For P-limiting case
        ...
        Qc=1.03*10**(-12)/12      #(molC/cell) biomass C per cell (196-18)(from Healey 1985)
    
    elif What_is_limiting==1:
    #For N-limiting case
        Pin=0.025  #(mol/m3) Phosphorus concentration in the incoming medium (Healey 1985)
        Nin=0.025    #(mol/m3) Nitrate concentration in the incoming medium (Healey 1985)
    ...
    Cnrna_variable=6212.592499         #(s) Constant for Variable part of RNA (193-26)
    ...
    Nconst_protein=1.18274979779916E-15
 ...
 #   Pconst_rna=Nconst_rna*YrnaN_P   #(molP cell-1) phosphorus in constant part of RNA (193-33)
    ...
    Qp_max=25/(3.097e16)                                                                              #(molP cell-1) total phosphorus content in the cell (193-26)
    ...
    for i in U:
        if Xn_max[i]>Xp_max[i]:
            ...
            Qp[i]=Qp_min[i]
            Qn_test[i]=Nin/X[i]
            Pstore[i]=Prna_variable[i]+Prna_const
   ...
   # Prna=Nrna*YrnaN_P           #(molP cell-1) phosphorus in RNA (193-33)
    ...
    #OOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOO
    BiomassC=12*X*Qc   #(mg C L-1) Biomass concentration
    NtoCplot=Qn/Qc   #(mol / mol) biomass N to C ratio (164-20)
    PtoCplot=Qp/Qc*30.97*10**6/(12*10**3)    #(ug P / mg C) biomass P to C ratio (164-20)
    NtoPplot=Qn/Qp*14*10**6/(30.97*10**6)        #(ug N /ug P) biomass N to P ratio (164-20)
    ChltoC0=Chl/Qc         #(mol C chl mol C -1) Chlorophyll to carbon ratio
    Mchl=893.49             #(g / mol chlorophyll) mollar mass of chlorophyll
   ...
   #         -Crna_const_plot-Crna_variable_plot-Cdna_const_plot-Cdna_variable_plot
    ...
    #=======================================
    Nphoto_plot=Nphoto/Qc   #(ug N/ mgC) Photosynthesis related protein nitrogen (193-25)(193-33)
    #Nproteinsynth_plot=Nproteinsynth*Nunit      #(ug N/ mgC) protein synthesis related protein in N (193-25)(193-33)
    Nbiosynth_plot=Nbiosynth/Qc      #(ug N/ mgC) biosynthesis related protein in N (193-37)
    Nconst_protein_plot=Nconst_protein*Numbertoarray/Qc    #(ug N/ mgC) constant protein pool in nitrogen (193-25)(193-33)
   # Nrna_plot=Nrna*Nunit        #(ug N/ mgC) nitrogen in RNA (193-33)
   # Ndna_plot=Ndna*Nunit        #(ug N/ mgC) nitrogen in DNA (193-33)
    Nchl_plot=Nchl/Qc        #(ug N/ mgC) Chlorophyll nitrogen (actually almost negligiable) (193-33)
#    Nconst_other_plot=Nconst_other*Nunit*Numbertoarray        #(ug N/ mgC) Other kinds of nitrogen assuming constant (193-33)
    
    Nrna_variable_plot=Nrna_variable/Qc      #(ug N/ mgC) Nitrogen in Variable part of nucleic acid (193-37)
    Ndna_variable_plot=Ndna_variable/Qc     #(ug N/ mgC) Nitrogen in Variable part of nucleic acid (193-37)
    
    Ndna_const_plot=Ndna_const*Numbertoarray/Qc    #(ug N/ mgC) Nitrogen in constant part of DNA
    Nrna_const_plot=Nrna_const*Numbertoarray/Qc    #(ug N/ mgC) Nitrogen in constant part of RNA
    
    Nstore_plot=Nstore     #(ug N/ mgC) Nitrogen in storage
    #=======================================
...
#OOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOO
    ...
    #============================================
    Xmax=1.8000001
    XTICKS=arange(0.0,1.5+0.5,0.5)

 
    if Plot_NC==1:
        figure(6)
        plot(Dd, NtoCplot, color=plotcolor,label=str(int(round(I,0)))+' $\mu$mol m$^{-2}$ s$^{-1}$')
        plot(data.Dnc, data.NC,'o' ,color=plotcolor)
        ...
        if What_is_limiting==0:
            ...
            ylim(ymax=0.30001)
        #ylim([0,801])
        #yticks(arange(0,801,100))
        xticks(XTICKS)
        legend(loc='upper right', borderaxespad=0.6, fontsize=24, ncol=1)
        xlim(xmax=Xmax)
        savefig("C:\\Users\\Keiin\\OneDrive\\desktop\\figures\\01\\Phytoplankton model\\"+Whatislimiting+"\\Chalup90\\N to C.png",dpi = DPI)
     
    #     ylim([0,21])
    #     yticks(arange(0,21,5))
    if Plot_Chl==1:
        figure(7)
        plot(Dd,ChltoCplot,color=plotcolor,label=str(int(I))+' $\mu$E m$^{-2}$ s$^{-1}$')
        #  plot(Dh,Dh,color=plotcolor)
        plot(data.Dchl, data.Chl,'o' ,color=plotcolor)
        ...
        xlim(xmax=Xmax)
   #     legend(loc='upper right', borderaxespad=0.6, fontsize=20, ncol=1)
         
        savefig("C:\\Users\\Keiin\\OneDrive\\desktop\\figures\\01\\Phytoplankton model\\"+Whatislimiting+"\\Chalup90\\Chl to C.png",dpi = DPI)
    ...
    StackPlotColorsC=(Color_Cessential,Color_Nstore,Color_protein_const,Color_photo,Color_protein_biosynthesis,Color_DNA,Color_RNA,Color_chl,Color_thylakoid,\
                      Color_other)
    StackPlotColorsCsmalls=(Color_DNA,Color_RNA,Color_chl,Color_thylakoid) 
    
    StackPlotColorsN=(Color_Other,Color_Photo,Color_Bio,Color_Nstore)
    ...
    if data.i>-1:
        if Plot_NC_stack==1:
            figure(FigurenumberN_C+data.i)
            stackplot(Dd,Nconst_protein_plot+Ndna_const_plot+Ndna_variable_plot,Nphoto_plot+Nchl_plot,Nbiosynth_plot+Nrna_const_plot+Nrna_variable_plot,Nstore_plot,colors=StackPlotColorsN)
    #        stackplot(Dd,Nconst_other_plot,Nconst_protein_plot,Nproteinsynth_plot,Nphoto_plot,Ndna_plot,Nrna_plot,Nchl_plot,colors=StackPlotColorsN)
            plot(data.Dnc, data.NC,'o' ,color='black')
        ...
        #     yticks(arange(0,26,5))
            ...
            xlim([lowlim,highlim])
            ylim([0,0.3000001])
            First_part="C:\\Users\\Keiin\\OneDrive\\desktop\\figures\\01\\Phytoplankton model\\"+Whatislimiting+"\\Chalup90\\N-C broad\\Fig."
            Figure_number=str(FigurenumberN_C+data.i)
            Last_part=".png"
            xlim(xmax=Xmax)
        ...
        if Plot_C_stack==1:
            ...
            xlabel('$\mu$ (d$^{-1}$)', fontsize=AxisFontSize)                       #copied from 73
            ylabel('C allocation ($\%$)', fontsize=AxisFontSize)  #copied from 73
        #     ylim([0,25])
        #     yticks(arange(0,26,5))
            xticks(XTICKS)
            xlim([lowlim,highlim])
            First_part="C:\\Users\\Keiin\\OneDrive\\desktop\\figures\\01\\Phytoplankton model\\"+Whatislimiting+"\\Chalup90\\C allocation broad\\Fig."
            Figure_number=str(FigurenumberC+data.i)
            Last_part=".png"
            ylim(ymax=percentorratio+1e-20)
            xlim(xmax=Xmax)
            title('Light: '+str(int(round(I,0)))+' $\mu$mol m$^{-2}$ s$^{-1}$',y=1.02)
            savefig(First_part+Figure_number+Last_part,dpi = DPI) 
            
             
             
 
        FigurenumberCsmalls=25
        if Plot_C_stack_small==1:
            figure(FigurenumberCsmalls+data.i)
          #  stackplot(Dd,Cconst_protein_plot,Cphoto_plot,Cbiosynth_plot,Cchl_plot,Cdna_const_plot,Cdna_variable_plot,\
          #            Crna_const_plot,Crna_variable_plot,Cother_plot,colors=StackPlotColorsC)
             
            stackplot(Dd,Cdna_const_plot+Cdna_variable_plot,\
                      Crna_const_plot+Crna_variable_plot,Cchl_plot,CthylakoidPG_plot,colors=StackPlotColorsCsmalls)
        ...
        #     yticks(arange(0,26,5))
            ...
            xlim([lowlim,highlim])
            First_part="C:\\Users\\Keiin\\OneDrive\\desktop\\figures\\01\\Phytoplankton model\\"+Whatislimiting+"\\C allocation smalls\\Fig."
            Figure_number=str(FigurenumberCsmalls+data.i)
            Last_part=".png"
            ylim(ymax=10)
            savefig(First_part+Figure_number+Last_part,dpi = DPI) 
        ...
        #Adding Mu max values==================================================
        
        Lightintensity=arange(0.1,2500+0.1,0.1)
        ...
        if data.i ==1:
            if Plot_NC==1:
                ...
                plot(Dmax*86400,NtoCplot/NunitData,':',color='black',zorder=-1,label='$\mu_{max}$')
                legend(loc='upper right', borderaxespad=0.6, fontsize=22, labelspacing=0.2, ncol=1)
                savefig("C:\\Users\\Keiin\\OneDrive\\desktop\\figures\\01\\Phytoplankton model\\"+Whatislimiting+"\\Chalup90\\N to C.png",dpi = DPI)
