...
def Plot1(FigNumber,Title,xData,yData,cData,xModel,yModel,cModel,xPchl,yPchl,cPchl,Loc):
    figure(str(FigNumber),figsize=(8,6.5))
    ax1=gca()
    ax2=ax1.twinx()
    ...
    ln3 = ax2.plot(xPchl,yPchl*24,'--',color=cPchl,label='$P_{Chl}$ $\ $model',zorder=1)
    ax1.set_ylim(0,1.8)
    ax2.set_ylim(0,0.003*24)
    title(Title)
    AxisFontSize = 30
    AxisFontSize2 = 26
    ax1.set_xlabel('Irradiance $\mu$mol m$^{-2}$ s$^{-1}$', fontsize = AxisFontSize)
    ax1.set_ylabel('$\mu_{max}$ (d$^{-1}$)', fontsize = AxisFontSize)
    ax2.set_ylabel('$P_{Chl}$ (mol C (mol C Chl)$^{-1}$ d$^{-1}$)', fontsize = AxisFontSize2)
    ax2.ticklabel_format(style='sci',axis='y',scilimits=(0,0))
    lns = ln1 + ln2 + ln3
    labs = [l.get_label() for l in lns]
    legend(lns, labs, loc=Loc,fontsize=23)
    xlim(0,200)
    
def Plot3(FigNumber,Title,xData,yData,cData,xModel,yModel,cModel,xPchl,yPchl,cPchl,Loc):
    figure(str(FigNumber),figsize=(8,6.5))
    ax1=gca()
    ax2=ax1.twinx()
    ln1 = ax1.plot(xData,yData,'o',color=cData,label='$\mu_{max}$ data',zorder=10)
    ...
    lns = ln1 + ln2 + ln3
    labs = [l.get_label() for l in lns]
    legend(lns, labs, loc=Loc,bbox_to_anchor=(0.7,0.55),fontsize=23)
    xlim(0,200)
...
#OOOOOOOOOOOOOOOOOOOOOOOOOOO
LoadFolderHealey85 = 'C:\\Users\\Keiin\\OneDrive\\Desktop\\figures\\01\\Phytoplankton model\\N-limiting\\MuMax\\'
LoadFolderChalup90 = 'C:\\Users\\Keiin\\OneDrive\\Desktop\\figures\\01\\Phytoplankton model\\N-limiting\\MuMax\\'
LoadFolderSakshaug89 = 'C:\\Users\\Keiin\\OneDrive\\Desktop\\figures\\01\\Phytoplankton model\\N-limiting\\MuMax\\'
...
Plot3('Chalup90','$P.$ $lutheri$',IDataChalup90,MuMaxDataChalup90,'g',IModelChalup90,MuMaxModelChalup90,'g',I,PchlC90,'#00FF00','lower right')
