...
#OOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOO
    ...
    if healey85data.i>-1:
        ...
        #     yticks(arange(0,26,5))
            ...
            xlim([lowlim,highlim])
            First_part="C:\\Users\\Keiin\\OneDrive\\Desktop\\figures\\01\\Phytoplankton model\\"+Whatislimiting+"\\Healey85\\C allocation broad\\Fig."
            Figure_number=str(FigurenumberC+healey85data.i)
            Last_part=".png"
            ylim(ymax=percentorratio+1e-20)
            title('Light: '+str(I)+' $\mu$mol m$^{-2}$ s$^{-1}$',y=1.02)
            savefig(First_part+Figure_number+Last_part,dpi=600) 
           
    DPI=600 
    if healey85data.i==2:         
        figure(100+healey85data.i)
        plot(Dd,Cphoto_plot+Cbiosynth_plot+Cconst_protein,'red',label='Protein')
        plot(Dd,Cessential_plot+Cother_plot+CthylakoidPG_plot,label='Carb. + Lipid')
        #title('Light: '+str(I)+' $\mu$mol m$^{-2}$ s$^{-1}$',y=1.02)
        title('Model: S. linearis',y=1.02)
        xlabel('$\mu$ (d$^{-1}$)')#, fontsize=30)
        ylabel('C allocation ($\%$)')
        ylim(ymax = 100)
        legend(loc=2,ncol=1,fontsize=20)
        savefig("C:\\Users\\Keiin\\OneDrive\\desktop\\figures\\01\\Phytoplankton model\\Felcmanova2017\\Model.png",dpi=DPI)
    return
